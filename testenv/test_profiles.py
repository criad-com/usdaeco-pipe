...
from usdaeco_pipe.profiles import apply_profile, load_profile
...
def test_validator_accepts_profile(tmp_path):
    from usdaeco_pipe import register_plugins, validators
    from pxr import Usd
    register_plugins()
    stage = Usd.Stage.CreateInMemory()
    prim = stage.DefinePrim("/Element", "Xform")
    prim.ApplyAPI("AecoPipeAPI")
    path = tmp_path / "exchange.json"
    path.write_text('{"include_builtin": false, "severity_overrides": {"pipeMissingAxis": "info"}}')
    issues = validators.validate_stage(stage, profile=path)
    matching = [e for e in issues if e.GetName() == "pipeMissingAxis"]
