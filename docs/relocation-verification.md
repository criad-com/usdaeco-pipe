...
|---|---|---|
| Full check.py | 82 checks, 0 failed | 82 checks, 0 failed |
...
in both layouts. The archived pipe source layer also opens independently
with no family plugins and zero composition errors after relocation.

Source pytest: **20 passed**. Exact integration pins are recorded in
...
Pipe changes only one source-reference line in its review layers; the manifest
records the new hash and toolchain pin.

The result totals **5,614,609 bytes**. The runtime `inputs/source` link is ignored
